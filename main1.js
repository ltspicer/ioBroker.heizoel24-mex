...
class Heizoel24Mex extends utils.Adapter {
    ...
    async onReady() {
        ...
        let sensor_id = 1;
        const reference_month_in = this.config.reference_month;
        let reference_month = 6;
        ...
        if (Number(reference_month_in)) {
            reference_month = parseInt(reference_month_in);
            if (reference_month < 1 || reference_month > 12) {
                this.log.warn('Reference month has no value between 1 and 12. Set it to 6');
                reference_month = 6;
            }
        } else {
            this.log.warn('Reference month has no value between 1 and 12. Set it to 6');
        ...
        this.log.debug(`Reference month is ${reference_month}`);
    ...
    async mainRoutine(client, username, passwort, mqtt_active, sensor_id, storeJson, storeDir) {
        ...
        if (storeJson) {
            ...
            if (this.oil_usage) {
                try {
                    const oilJson = JSON.stringify(this.oil_usage, null, 4);
        ...
        if (this.oil_usage && this.oil_usage['Values']) {
            const oilUsage = this.oil_usage['Values'];
            let oilJsonData = '[\n';
            let count = 0;
